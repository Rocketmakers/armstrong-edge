...
export namespace MemoryServer {
  ...
  export function searchUsers(search: string): IUser[] {
    ...
    return users.filter(
      (u) =>
        regex.test(u.firstName) || regex.test(u.lastName) || regex.test(u.email)
    );
  }
  ...
  export function getSauces(q: string) {
    return q?.length
      ? sauces.filter((sauce) => sauce.name.includes(q))
      : sauces;
  }
}
