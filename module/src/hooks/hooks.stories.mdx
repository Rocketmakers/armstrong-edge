...
<Meta title="Utilities/Hooks/Hooks" />
...
const [{ height, width, bottom, left, right, top, x, y }, forceResize] = useBoundingClientRect(contentRef);
...
```tsx
const [state, setState] = useTemporaryState(initialValue, time, onReset);
```
...
```tsx
const { set, clear } = useTimeout(timeoutTime, onTimeout);
```
