...
const [{ height, width, bottom, left, right, top, x, y }] =
...
```tsx
const [actualValue, setActualValue, debouncedValue, setDebouncedValue] =
  useDebounce(milliseconds, value, onValueChange);
```
...
const MyComponent = ({ value, onChange }) => {
  const [internaValue, setInternalValue] = useOverridableState(
    "",
    value,
    onChange
  );
...
```tsx
const [state, setState, cancel] = useTemporaryState(
  initialValue,
  time,
  onReset
);
...
```tsx
const [actualValue, setActualValue, throttleValue, setThrottledValue] =
  useThrottle(milliseconds, value, onValueChange);
```
...
```tsx
const { waiting: hasClickedTheThing, set: onClickTheThing } = useTimeout(
  undefined,
  1000
);
