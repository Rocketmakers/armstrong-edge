...
describe("useResizeObserver", () => {
  ...
  it("should observe the specified element", async () => {
    ...
    const { result } = renderHook(() =>
      useResizeObserver(callback, options, refMock)
    );
    const { observer } = result.current;
    await waitFor(() => expect(observer.current).toBeDefined());
    expect(observer.current?.observe).toHaveBeenCalledWith(
      refMock.current,
      options
    );
  });
  ...
  it("should unobserve the specified element", async () => {
    ...
    const { unmount, result } = renderHook(() =>
      useResizeObserver(callback, undefined, refMock)
    );
    const { observer } = result.current;
    await waitFor(() => expect(observer.current).toBeDefined());
    unmount();
  ...
  it("should not create an observer if ref is not defined", async () => {
    ...
    await waitFor(() =>
      expect(result.current?.observer).toEqual({ current: undefined })
    );
  });
