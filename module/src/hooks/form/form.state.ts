...
export function dataReducer<TData extends object>(
  state: TData,
  action: FormAction<TData, any>
): TData {
  switch (action.type) {
    case "set-one":
      return (
        Array.isArray(state) || Number.isInteger(action.propertyKey)
          ? ((state as any[]) || []).map((_, i) =>
              i === action.propertyKey ? action.value : state[i]
            )
          : { ...(state || {}), [action.propertyKey]: action.value }
      ) as TData;
    case "set-path":
      return Objects.mergeDeepFromKeyChain(
        state,
        action.keyChain,
        action.value
      );
      ...
      const validationToClear = state.filter((e) => {
        ...
        return (
          ...
          (!action.identifiers?.length ||
            action.identifiers.some((id) => id === e.identifier))
        );
      });
