...
describe("useDebounce", () => {
  ...
  it("resets a value back to its original value if a reset is called", async () => {
    ...
    const newValue = 'value';
    const onChange = jest.fn(() => null);
    const { result, rerender } = renderHook((prop) => useDebounce(ms, value, onChange));
  ...
  it("force sets a new value if the hard value changes", async () => {
    const ms = 600;
    let value = 'test';
    const newValue = 'value';
    const onChange = jest.fn(() => null);
    const { result, rerender } = renderHook(() => useDebounce(ms, value, onChange));
