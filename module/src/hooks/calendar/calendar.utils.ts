import { addDays, addMonths, format, getDaysInMonth, isAfter, isBefore, isSameDay, Locale, startOfYear } from 'date-fns';

import { Dates } from '../../utils/dates';
import { ICalendarHighlightParsed, IDay, IMonth } from './calendar.types';
...
export function getMonths(forYear: number, min?: Date, max?: Date, locale: Locale = Dates.defaultLocale): IMonth[] {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    months.push({
      ...
      shortName: format(currentDate, 'MMM', { locale }),
      indexInYear: i,
