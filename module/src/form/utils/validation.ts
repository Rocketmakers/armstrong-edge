...
export function validationErrorsByKeyChain(
  ...
  const keyChainAttrStringSquareArray = keyStringFromKeyChain(
    keyChain,
    "brackets"
  );
 ...
 */
export function clientValidationReducer(
  state: IValidationError[],
        ...
        (s) =>
          !(
            ...
            (!action.identifiers?.length ||
              action.identifiers?.some((i) => i === s.identifier))
          )
      );
    default:
      return state;
 ...
 */
export function zodFromValidationSchema<TData>(
  schema: IRootValidationSchema<TData>
) {
  // type checker utility - is TS validation field an array type
  const isArrayOfZod = (
    arrayItem: unknown
  ): arrayItem is IArrayOfZod<unknown> => {
  ...
  const isObjectOfZod = (
    objectItem: unknown
  ): objectItem is IObjectOfZod<unknown> => {
  ...
  const unpackObject = (incomingObject: object) => {
    return Object.keys(incomingObject).reduce(
      (sch, key) => ({
        ...
        [key]: unpackValueToZod(
          incomingObject[key as keyof typeof incomingObject]
        ),
      }),
      {}
    );
  ...
  const unpackValueToZod = (incomingToZod: unknown): unknown => {
    ...
    if (isObjectOfZod(incomingToZod)) {
      const obInner = z.object(
        unpackObject(incomingToZod.schema as z.ZodRawShape)
      );
  ...
  if (isArrayOfZod(schema) || isObjectOfZod(schema)) {
    return unpackValueToZod(schema) as
      | z.ZodArray<z.ZodAny>
...
export const getMyZodErrors = (errors: ZodIssue[], keyChainString?: string) => {
  return errors
    .filter((e) =>
      keyChainString
        ? isMyKeyChainItem(
            keyStringFromKeyChain(e.path, "dots"),
            keyChainString
          )
        : true
    )
