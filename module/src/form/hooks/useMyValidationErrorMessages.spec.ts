...
describe('useMyValidationErrorMessages', () => {
  ...
  it('should return the bind validation errors when they are provided', () => {
    const { result } = renderHook(() => useMyValidationErrorMessages(bindingPropsMock));
  ...
  it('should return unique validation error messages from both sources', () => {
    const validationErrorMessages = ['bind error 1', 'a second error', 'error the third'];
    const { result } = renderHook(() => useMyValidationErrorMessages(bindingPropsMock, validationErrorMessages));
    expect(result.current).toEqual(['bind error 1', 'a second error', 'error the third', 'bind error 2']);
  });
});
