...
```ts
function arrayToDictionary<T, Keys extends string = string>(
  array: T[],
...
```ts
function arrayToArraysByKey<T, Keys extends string = string>(
  array: T[],
  getKey: (item: T) => Keys
);
```
