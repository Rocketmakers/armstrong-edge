...
export const Bound: Story = {
  ...
  render: () => {
    ...
    return (
      <>
        <TextArea
          testId={"bound-text-area"}
          label="Bound text area"
          bind={formProp("text").bind()}
        />
        <p data-testid={"bound-result"}>Value: {formState?.text}</p>
        <TextArea
          label="Bound debounce text area (400ms)"
  ...
  play: async ({ canvasElement }) => {
    ...
    const debounceTextArea = canvas.getByLabelText(
      "Bound debounce text area (400ms)"
    );
