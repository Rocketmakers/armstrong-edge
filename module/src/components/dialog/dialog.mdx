...
If you're building a dialog component to be used throughout your app, the `useDialog` hook has you covered. Simply write your component with a `ref` prop and pass the ref into the `useDialog` hook.

**NOTE:** Reusable dialogs can still use the utility function returned from the `useDialog` hook just like the earlier example.

```tsx
export const ReusableDialog = ({ ref }: { ref?: React.Ref<DialogElement> }) => {
