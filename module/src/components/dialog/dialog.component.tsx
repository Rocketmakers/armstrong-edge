 ...
 */
export const Dialog = ((
  props: React.PropsWithChildren<IDialogProps<unknown>> & { ref?: React.Ref<DialogElement<unknown> | null> }
) => {
  const {
    ref,
    children,
    ...
    <RadixDialog.Root open={visible} onOpenChange={onInnerOpenChange}>
      {globals.globalPortalTo &&
        ReactDOM.createPortal(
          <RadixDialog.Overlay className={concat('arm-dialog-overlay', overlayClassName)} data-visible={!finishAction}>
            <RadixDialog.Content
