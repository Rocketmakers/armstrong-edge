...
export const NoCloseButton: StoryObj<typeof Dialog> = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(
      queryByRole(dialog, "button", { name: "Close" })
...
const ReusableDialogExample = React.forwardRef<DialogElement>((props, ref) => {
  ...
  return (
    <Dialog ref={dialogRef} title="Reusable dialog">
      <div>
        This custom dialog can be used throughout the app just like an Armstrong
        dialog!
      </div>
      <Button
        onClick={cancel}
...
const LoginDialog = React.forwardRef<DialogElement<ILoginData>>(
  (props, ref) => {
...
export const ReusableFormDialog: StoryObj<typeof Dialog> = {
  render: () => {
    ...
    const openDialog = React.useCallback(async () => {
      ...
      setResult(
        `${action}, username: ${data?.username}, password: ${data?.password}`
      );
    }, [open, setResult]);
  ...
  play: async ({ canvasElement }) => {
    ...
    // fill inputs
    const username =
      within(dialog).getByPlaceholderText<HTMLInputElement>("Username");
    await userEvent.type(username, testUsername);
    const password =
      within(dialog).getByPlaceholderText<HTMLInputElement>("Password");
    ...
    await waitFor(() =>
      expect(result).toHaveTextContent(
        `ok, username: ${testUsername}, password: ${testPassword}`
      )
    );
  },
};
