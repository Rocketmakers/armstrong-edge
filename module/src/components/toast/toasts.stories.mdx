...
The hook `useToasts` will return an array of dispatched toast notifications. You will need to reimplement a fair few things yourself to replicate all the functionality of useToasts, like `position` and `autoDismissTime`.

`useToastConfig` will allow you to access configuration set at the Provider level (or default configurations)
