...
export const Grouped: StoryObj<typeof CheckboxInputList> = {
          ...
          options={[
            {
              ...
              content: "red",
              group: "primary",
              rightIcon: IconUtils.getIconDefinition("Icomoon", "cheese"),
            },
            { id: 2, content: "blue", group: "primary" },
            {
              ...
              content: "pink",
              group: "secondary",
              rightIcon: IconUtils.getIconDefinition("Icomoon", "heart"),
            },
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(group1?.nextSibling?.nextSibling).toContainElement(
      canvas.getByText("blue")
    );
    const group2 = canvas.getByText("secondary").parentElement;
    expect(group2?.nextSibling).toContainElement(canvas.getByText("pink"));
    expect(group2?.nextSibling?.nextSibling).toContainElement(
      canvas.getByText("brown")
    );
  },
};
...
export const CustomIcons: StoryObj<typeof CheckboxInputList> = {
        ...
        <CheckboxInputList
          ...
          options={[
            ...
            { id: 2, content: "blue" },
            {
              ...
              content: "pink",
              rightIcon: IconUtils.getIconDefinition("Icomoon", "heart"),
              leftIcon: IconUtils.getIconDefinition("Icomoon", "heart"),
            },
            { id: 4, content: "brown" },
          ]}
        />
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const pink = canvas.getByText("pink").parentElement
      ?.parentElement as HTMLElement;
    const red = canvas.getByText("red").parentElement
      ?.parentElement as HTMLElement;
...
export const CustomContentWithoutCheckbox: StoryObj<typeof CheckboxInputList> =
  {
    ...
    play: async ({ canvasElement }) => {
      const canvas = within(canvasElement);
      const checked = canvas.getByText(`I'm checked`).parentElement
        ?.parentElement as HTMLElement;
      const unchecked = canvas.getAllByText(`I'm not checked`)[0].parentElement
        ?.parentElement as HTMLElement;
      expect(checked).toHaveAttribute("data-checked", "true");
      expect(within(checked).getByRole("img")).toHaveAttribute(
        "src",
        checkedUrl
      );
      expect(unchecked).toHaveAttribute("data-checked", "false");
      expect(within(unchecked).getByRole("img")).toHaveAttribute(
        "src",
        uncheckedUrl
      );
    },
