...
export const Bound: Story = {
  ...
  render: () => {
    ...
    return (
      <>
        ...
        <p data-testid={"bound-result"}>
          Bound value is {formState?.checked ? "checked" : "not checked"}
        </p>
      </>
    );
  },
  play: async ({ canvasElement }) => {
    ...
    userEvent.click(switchInput);
    await waitFor(() =>
      expect(canvas.getByTestId("bound-result")).toHaveTextContent(
        "Bound value is checked"
      )
    );
  },
};
