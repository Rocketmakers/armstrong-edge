...
You must have the icon font file for the icon font you're using.

If you are internal to Rocketmakers, talk to tech-web or an Armstrong person, and we'll hook you up with a font file.

If you are external, then to use icon features, you will need a licence to one of these font files - though in most places, icons can be replaced with custom JSX
...
```scss
@import "~@rocketmakers/armstrong/dist/icomoon.scss";
@import "~@rocketmakers/armstrong/dist/linearicons.scss";
```
...
From here, icons can be used in Icon components

```tsx
<Icon iconSet="IcoMoon" iconName="heart2" />
// or
<IcomoonIcon iconName="heart2" />
// or
<LinearIcon iconName="tap" />
```
