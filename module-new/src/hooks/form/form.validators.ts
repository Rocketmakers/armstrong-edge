...
function isValidator<TValue>(val: unknown): val is IClientValidatorFieldConfig<TValue> {
  return !!(val as IClientValidatorFieldConfig<TValue>)?.validator;
}
...
export function validateKeyChainProperty<TData>(
  ...
  if (!isValidator(validatorConfig)) {
    ...
    const config = valueByKeyChain<ClientValidationConfig<TData>, IClientValidatorFieldConfig<unknown>>(
      validatorConfig,
      keyChain
    );
    const state = valueByKeyChain(formState, keyChain);

    if (!keyChain.length && config) {
