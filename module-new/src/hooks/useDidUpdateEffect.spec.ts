...
describe('useDidUpdateLayoutEffect', () => {
  it('should run the effect callback when the dependencies change', () => {
    const effectCallback = jest.fn();

    const { rerender } = renderHook(deps => useDidUpdateLayoutEffect(effectCallback, deps), {
      initialProps: [1, 2, 3], // Initial dependencies
    });
  ...
  it('should not run the effect callback on the first render', () => {
    ...
    renderHook(() => useDidUpdateLayoutEffect(effectCallback, [1, 2, 3]));
